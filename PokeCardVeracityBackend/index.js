...
const bcakg = "../PokeCardVeraCity/src/assets/images";
const fs = require('fs');
const tf = require('@tensorflow/tfjs-node');

// async function loadModel() {
//   // Replace this with the path to your model.json file
//   const modelPath = 'file://C:/node/PokeCardVeracityBackend/tf/model.json';
...
// app.post('/single', upload.single('bimg'), async (req, res) => {
//     try {
//       // Get the uploaded file
//       const file = req.file;
  
//       // Load the model
//       const modelPath = 'file://C:/node/PokeCardVeracityBackend/ngx/model.json';
//       const model = await tf.loadLayersModel(modelPath);
...
//       const preprocessedImage = await sharp(imageData)
//         .resize({ width: 256, height: 256 })
//         .toBuffer();
  
//       // Decode the preprocessed image data and convert it to a tensor
//       const imageTensor = tf.node.decodeImage(preprocessedImage);
  
//       // Reshape the image tensor to match the desired input shape
//       const reshapedImage = tf.reshape(imageTensor, [1, 256, 256, 3]);
  
//       // Normalize the image data if necessary
//       const normalizedImage = reshapedImage.div(255.0);
  
//       // Make a prediction using the model
//       const prediction = model.predict(normalizedImage);
  
//       // Convert the prediction to a JavaScript array and send it as a response
...
app.post('/single', upload.single('bimg'), async (req, res) => {
    try {
      ...
      const mean = normalizedImage.mean();
      const squaredDiff = normalizedImage.sub(mean).square();
      ...
      // Make a prediction using the model
      const prediction = model.predict(standardizedImage);
  
      // Convert the prediction to a JavaScript array
      const predictionArray = prediction.arraySync();
      
      // Convert the numerical prediction to a binary classification using a threshold of 0.5
      const threshold = 0.3
      const binaryPrediction = predictionArray[0][0] > threshold ? "genuine" : "not genuine";
      
      // Send the binary classification as a response
      res.json(binaryPrediction);
    } catch (error) {
      console.error(error);
      res.status(500).json({ error: 'Something went wrong' });
    }
